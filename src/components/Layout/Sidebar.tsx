    ...
    return (
      <div>
        <NavLink
          to={item.href}
          className={({ isActive }) =>
            `group flex items-center px-2 py-2 text-sm font-medium rounded-md transition-colors duration-200 nav-item ${
              level === 0 ? 'pl-3' : 'pl-8'
            } ${
              isActive
        ...
        >
          {item.emoji && <span className="mr-2 text-lg flex-shrink-0">{item.emoji}</span>}
          <item.icon
            className={`mr-3 flex-shrink-0 h-5 w-5 icon ${
              level === 0 ? '' : 'h-4 w-4'
            }`}
            aria-hidden="true"
          />
          ...
          {item.badge && (
            <span className="ml-auto inline-block py-0.5 px-2 text-xs font-medium rounded-full bg-primary-100 text-primary-600 flex-shrink-0">
              {item.badge}
            </span>
          )}
        </NavLink>
