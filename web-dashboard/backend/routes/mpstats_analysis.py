...
async def get_mpstats_similar_items(article: str, date_from: str, date_to: str, fbs: int = 1, max_retries: int = 3) -> Optional[Dict]:
    """
    Получает данные о похожих товарах через MPStats API /get/in_similar
    """
    # Проверяем, что API ключ настроен
    if not MPSTATS_API_KEY or MPSTATS_API_KEY == "your_mpstats_api_key_here":
    ...
    params = {
        "d1": date_from,
        "d2": date_to,
        "path": str(article),  # Используем артикул как path, преобразуем в строку
        "fbs": fbs
    }
    
    logger.info(f"🔍 MPStats similar items request: {url}")
    logger.info(f"📋 Request params: {params}")
    logger.info(f"🔑 Using API key: {MPSTATS_API_KEY[:10]}...")
    
    for attempt in range(max_retries):
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers, params=params, timeout=30) as resp:
                    logger.info(f"📊 MPStats similar items response (attempt {attempt + 1}): {resp.status}")
                    ...
                    if resp.status == 200:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            logger.warning(f"⚠️ MPStats API rate limit (429), waiting {wait_time}s before retry {attempt + 2}/{max_retries}...")
                            await asyncio.sleep(wait_time)
                            continue
                        else:
                            logger.error(f"❌ MPStats API rate limit after {max_retries} attempts")
                            return None
                    elif resp.status == 405:
                        logger.error(f"❌ MPStats API method not allowed (405) - check API endpoint")
                        return None
                    else:
            ...
            logger.error(f"❌ MPStats API timeout after 30 seconds (attempt {attempt + 1})")
            if attempt < max_retries - 1:
                await asyncio.sleep(1)
                continue
            return None
        except Exception as e:
            logger.error(f"❌ MPStats API exception (attempt {attempt + 1}): {e}")
            if attempt < max_retries - 1:
                await asyncio.sleep(1)
                continue
            return None
    
    logger.error(f"❌ MPStats API failed after {max_retries} attempts")
    return None
...
def extract_pricing(item: Dict) -> Dict[str, Any]:
    """Извлекает информацию о ценообразовании с валидацией"""
    if not item:
        return {}
    ...
    # Валидация цен - если скидка больше 80%, то корректируем данные
    if final_price > 0 and start_price > 0:
        discount = ((start_price - final_price) / start_price) * 100
        if discount > 80:
            logger.warning(f"🚨 Unrealistic discount detected: {discount:.1f}% (from {start_price} to {final_price}), adjusting...")
            # Корректируем start_price чтобы скидка была разумной (максимум 50%)
            start_price = final_price * 2  # 50% скидка максимум
...
async def get_advanced_product_analysis(request: AdvancedProductAnalysisRequest):
    ...
    # Проверяем, что target_item существует
    if not target_item:
        logger.warning(f"⚠️ No target item found for article {request.article}")
        target_item = {}  # Создаем пустой словарь для избежания ошибок
