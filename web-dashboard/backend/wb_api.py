        ...
        async with aiohttp.ClientSession() as session:
            ...
            product_info = {
                ...
                "supplier_info": {
                    ...
                    "name": product_data.get("supplierName", "Не указан")
                },
                "price": {
...
async def generate_ai_recommendations(product_info: Dict[str, Any]) -> list:
    ...
    try:
        ...
        sales_data = product_info.get('sales', {})
        price_data = product_info.get('price', {})
        analytics_data = product_info.get('analytics', {})
        
        # Анализируем производительность товара
        daily_sales = sales_data.get('today', 0)
        daily_revenue = sales_data.get('revenue', {}).get('daily', 0)
        rating = product_info.get('rating', 0)
        reviews_count = product_info.get('reviews_count', 0)
        stocks = product_info.get('stocks', {}).get('total', 0)
        
        # Определяем проблемные зоны
        issues = []
        if daily_sales == 0:
            issues.append("нет продаж")
        if rating < 4.0:
            issues.append("низкий рейтинг")
        if reviews_count < 50:
            issues.append("мало отзывов")
        if stocks == 0:
            issues.append("нет остатков")
        elif stocks > 500:
            issues.append("избыток товара")
        ...
        if reviews_count > 100:
            strengths.append("много отзывов")
        if daily_sales > 5:
            strengths.append("хорошие продажи")
        if daily_revenue > 10000:
            strengths.append("высокая выручка")
            
        context = f"""
        ПОЛНЫЙ АНАЛИЗ ТОВАРА WILDBERRIES:
        
        === ОСНОВНАЯ ИНФОРМАЦИЯ ===
        📦 Товар: {product_info.get('name', 'Не указано')}
        🏷️ Бренд: {product_info.get('brand', 'Не указано')} 
        ...
        === ЦЕНООБРАЗОВАНИЕ ===
        💰 Текущая цена: {price_data.get('current', 0):,} руб
        💵 Оригинальная цена: {price_data.get('original', 0):,} руб
        📉 Скидка: {price_data.get('discount', 0)}%
        💎 Средняя цена в категории: {price_data.get('average', 'не указано')}
        
        === ПРОДАЖИ И ВЫРУЧКА ===
        📊 Продажи в день: {daily_sales} шт
        📈 Продажи в неделю: {sales_data.get('weekly', 0)} шт
        📅 Продажи в месяц: {sales_data.get('monthly', 0)} шт
        🎯 Всего продаж: {sales_data.get('total', 0)} шт
        💰 Выручка в день: {daily_revenue:,} руб
        💵 Выручка в месяц: {sales_data.get('revenue', {}).get('monthly', 0):,} руб
        💎 Прибыль в день: {sales_data.get('profit', {}).get('daily', 0):,} руб
        
        === РЕЙТИНГ И ОТЗЫВЫ ===
        ⭐ Рейтинг: {rating}/5
        📝 Отзывов: {reviews_count:,} шт
        📊 Конверсия в покупку: {analytics_data.get('conversion_rate', 'не указано')}%
        🛒 Процент выкупа: {analytics_data.get('purchase_rate', 'не указано')}%
        
        === ТОВАРНЫЕ ОСТАТКИ ===
        📦 Общие остатки: {stocks:,} шт
        📏 Размеров в наличии: {len(product_info.get('stocks', {}).get('by_size', {}))}
        🎨 Цветов: {product_info.get('colors_info', {}).get('total_colors', 1)}
        ⏰ Дней оборачиваемости: {analytics_data.get('turnover_days', 'не указано')}
        
        === КОНКУРЕНТНАЯ ПОЗИЦИЯ ===
        🏆 Позиция в поиске: {analytics_data.get('avg_position', 'не указано')}
        📊 Доля рынка: {analytics_data.get('market_share', 'не указано')}%
        💼 Сезонный фактор: {analytics_data.get('seasonal_factor', 'не указано')}
        
        === ПРОБЛЕМНЫЕ ЗОНЫ ===
        {', '.join(issues) if issues else 'Серьезных проблем не выявлено'}
        
        === СИЛЬНЫЕ СТОРОНЫ ===
        {', '.join(strengths) if strengths else 'Требуется детальный анализ'}
        """
        ...
        response = await asyncio.get_event_loop().run_in_executor(
            ...
            lambda: client.chat.completions.create(
                ...
                {
                    "role": "system", 
                    "content": """Ты профессиональный консультант по маркетплейсу Wildberries с 10+ летним опытом. 
                    
                    Твоя задача: провести глубокий анализ товара и дать МАКСИМАЛЬНО ПОЛЕЗНЫЕ, ПРАКТИЧНЫЕ рекомендации для роста продаж.
                    
                    ОБЯЗАТЕЛЬНО включи рекомендации по следующим направлениям:
                    🎯 ОПТИМИЗАЦИЯ КАРТОЧКИ: фото, описание, ключевые слова
                    💰 ЦЕНОВАЯ СТРАТЕГИЯ: позиционирование цены, скидки, промо
                    📦 УПРАВЛЕНИЕ ОСТАТКАМИ: поставки, размерный ряд
                    📈 УВЕЛИЧЕНИЕ ПРОДАЖ: реклама, акции, сезонность  
                    ⭐ РАБОТА С ОТЗЫВАМИ: как получить больше положительных отзывов
                    🏆 КОНКУРЕНТНОЕ ПРЕИМУЩЕСТВО: как обойти конкурентов
                    
                    Дай 8-12 конкретных, применимых рекомендаций. Каждая рекомендация должна быть:
                    - Конкретной (что именно делать)
                    - Измеримой (ожидаемый результат)
                    - Реализуемой (как это сделать)
...
def generate_fallback_recommendations(product_info: Dict[str, Any]) -> list:
    """Генерирует расширенные профессиональные рекомендации на основе анализа данных товара"""
    ...
    daily_sales = sales.get('today', 0)
    total_stock = stocks.get('total', 0)
    daily_revenue = sales.get('revenue', {}).get('daily', 0)
    ...
    if total_stock == 0:
        recommendations.append("📦 СРОЧНО: Пополните остатки в течение 3-5 дней! Товар без остатков теряет 50-70% позиций в поиске")
    
    # ⭐ РАБОТА С СОЦИАЛЬНЫМИ ДОКАЗАТЕЛЬСТВАМИ  
    if rating < 4.0:
        recommendations.extend([
            f"⭐ Рейтинг {rating}/5 критически влияет на конверсию. Изучите ТОП-5 негативных отзывов и устраните проблемы",
            "🎁 Отправляйте компенсационные подарки недовольным клиентам с просьбой пересмотреть оценку"
        ])
    
    if reviews < 50:
        recommendations.extend([
            f"📝 Мало отзывов ({reviews}). ЦЕЛЬ: 100+ отзывов за 2 месяца для повышения доверия на 40%",
            "💌 Настройте автоматическое SMS через 7 дней после доставки: 'Оцените товар, получите скидку 10% на следующую покупку'"
        ])
    
    # 💰 СТРАТЕГИЯ ЦЕНООБРАЗОВАНИЯ
    if daily_sales < 3 and current_price > 0:
        recommendations.append(f"💰 При продажах {daily_sales}/день протестируйте снижение цены на 15%. Цель: увеличить продажи в 2-3 раза")
    
    if discount > 50:
        recommendations.append("⚠️ Скидка >50% воспринимается как 'некачественный товар'. Оптимально: 15-35%")
    elif discount == 0 and daily_sales < 5:
        recommendations.append("🏷️ Добавьте скидку 20-25% на 2 недели для стимулирования первых продаж и сбора отзывов")
    
    # 📦 ЛОГИСТИКА И ОСТАТКИ
    if total_stock > 500:
        recommendations.extend([
            f"⚖️ Избыток товара ({total_stock:,} шт). Проведите flash-распродажу со скидкой 30% на 5-7 дней",
            "📊 Рассчитайте ABC-анализ: если оборачиваемость >60 дней, сократите следующий заказ на 40%"
        ])
    elif total_stock < 50 and daily_sales > 2:
        recommendations.append("📈 При стабильных продажах заказывайте товар с запасом на 45-60 дней для избежания дефицита")
    ...
    if daily_revenue < 5000:
        recommendations.extend([
            "🚀 Настройте автокампанию 'Поиск + Категории' с бюджетом от 1% от планируемой выручки",
            "📱 Создайте контент-план: фото товара в использовании, отзывы клиентов, сравнение с аналогами"
        ])
    
    # 🏆 КОНКУРЕНТНАЯ СТРАТЕГИЯ
    recommendations.extend([
        "🔍 Еженедельный мониторинг: отслеживайте цены и акции у ТОП-5 конкурентов через сервисы аналитики",
        "📊 Контролируйте позицию в поиске по 10 ключевым запросам, цель: войти в ТОП-20 по всем запросам",
        "💡 Найдите УТП (уникальное торговое предложение): бесплатная доставка, подарок, гарантия, быстрая доставка"
    ])
    
    # 🎉 ДОПОЛНИТЕЛЬНЫЕ ВОЗМОЖНОСТИ
    if reviews > 20 and rating > 4.0:
        recommendations.append("🌟 У вас хорошая база! Используйте отзывы в рекламных материалах и создайте видео-обзоры")
    
    return recommendations[:12]  # Возвращаем до 12 профессиональных рекомендаций
